...
namespace NATSParsers {
     ...
     * @param error_desc Output: error description if failed
     * @return true if ACK parsed successfully, false if error response or parse failure
     * @note Caller must free() the returned stream and error_desc strings
     */
    inline bool parse_pub_ack(const char* data, int size,
        ...
        if (seq != NULL && *seq == 0 && stream != NULL && *stream == NULL) {
            cJSON_Delete(root);
            return false;  // Neither stream nor seq found — invalid ACK
        }
     ...
     * @param error_desc Output: error description string
     * @return true if the response IS an error
     * @note Caller must free() the returned error_desc string
     */
