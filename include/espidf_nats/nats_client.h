...
class NATS
{
  ...
  public:
     ...
     */
    NATS(const char *hostname, int port = NATS_DEFAULT_PORT, const char *user = NULL, const char *pass = NULL,
         const nats_tls_config_t *tls_cfg = NULL)
        : transport(NULL), owns_transport(false), sockfd(-1), tls(NULL), mbedtls_ssl(NULL), mbedtls_conf(NULL),
          mbedtls_cacert(NULL), mbedtls_clicert(NULL), mbedtls_pkey(NULL),
#if MBEDTLS_VERSION_NUMBER < 0x04000000
          mbedtls_entropy(NULL), mbedtls_ctr_drbg(NULL),
#endif
          using_mbedtls_directly(false), current_server_idx(0), user(user), pass(pass), ping_timer(NATS_PING_INTERVAL),
          reconnect_timer(NATS_RECONNECT_INTERVAL), outstanding_pings(0), reconnect_attempts(0),
          last_reconnect_attempt(0), connect_state(DISCONNECTED), async_connect_cb(NULL), log_tick_count(0),
          last_error_code(NATS_ERR_NONE), connected(false), max_outstanding_pings(3), max_reconnect_attempts(-1),
          exponential_backoff_enabled(true), message_buffering_enabled(true),
          max_pending_messages(NATS_MAX_PENDING_MESSAGES), on_connect(NULL), on_disconnect(NULL), on_error(NULL)
    {
     ...
     *       const char[] arrays or embedding certificates in flash memory.
     */
    NATS(const nats_server_t *server_list, size_t server_count, const char *user = NULL, const char *pass = NULL,
         const nats_tls_config_t *tls_cfg = NULL)
        : transport(NULL), owns_transport(false), sockfd(-1), tls(NULL), mbedtls_ssl(NULL), mbedtls_conf(NULL),
          mbedtls_cacert(NULL), mbedtls_clicert(NULL), mbedtls_pkey(NULL),
#if MBEDTLS_VERSION_NUMBER < 0x04000000
        ...
        if (server_list != NULL && server_count > 0) {
            for (size_t i = 0; i < server_count; i++) {
                ...
                if (server_list[i].port < 1 || server_list[i].port > 65535) {
                    ESP_LOGE(tag, "Invalid port number for server %zu: %d (must be 1-65535)", i, server_list[i].port);
                    last_error_code = NATS_ERR_INVALID_ARG;
                    // Skip this server
                    continue;
                }
            ...
            ESP_LOGE(tag, "Invalid server configuration: server_list=%p, count=%zu", server_list, server_count);
            last_error_code = NATS_ERR_INVALID_ARG;
        }
        // Validate credential lengths (Issue #31)
        if (user != NULL && strlen(user) > NATS_MAX_CREDENTIAL_LEN) {
     ...
     */
    static NATS *create_tcp(const char *hostname, int port = NATS_DEFAULT_PORT, const char *user = NULL,
                            const char *pass = NULL, const nats_tls_config_t *tls_cfg = NULL)
    {
     ...
     * @return Pointer to new NATS instance (caller owns, delete when done)
     */
    static NATS *create_tcp(const nats_server_t *server_list, size_t server_count, const char *user = NULL,
                            const char *pass = NULL, const nats_tls_config_t *tls_cfg = NULL)
    {
     ...
     */
    static NATS *create_websocket(const char *hostname, int port, const char *user = NULL, const char *pass = NULL,
                                  const nats_tls_config_t *tls_cfg = NULL, const char *ws_path = NATS_WEBSOCKET_PATH)
    {
     ...
     */
    static NATS *create_websocket_uri(const char *uri, const char *user = NULL, const char *pass = NULL,
                                      const nats_tls_config_t *tls_cfg = NULL)
    {
     ...
     * @return Pointer to new NATS instance, or NULL on failure
     */
    static NATS *create_websocket(const nats_server_t *server_list, size_t server_count, const char *user = NULL,
                                  const char *pass = NULL, const nats_tls_config_t *tls_cfg = NULL)
    {
        if (server_list == NULL || server_count == 0) {
  ...
  private:
    ...
    void send_connect()
    {
        ...
        int offset = snprintf(buf, buf_size,
                              ...
                              "\"headers\":true,"
                              "\"no_responders\":true",
                              NATS_CONF_VERBOSE ? "true" : "false", NATS_CONF_PEDANTIC ? "true" : "false",
                              NATS_CLIENT_LANG, NATS_CLIENT_VERSION);
        ...
        switch (auth_config.method) {
            case NATS_AUTH_USER_PASS:
                if (user != NULL && pass != NULL) {
                    ...
                    int n = snprintf(buf + offset, buf_size - offset, ",\"user\":\"%s\",\"pass\":\"%s\"", escaped_user,
                                     escaped_pass);
            ...
            case NATS_AUTH_TOKEN:
                if (auth_config.token != NULL) {
                    int n = snprintf(buf + offset, buf_size - offset, ",\"auth_token\":\"%s\"", auth_config.token);
                    if (n < 0 || (size_t)n >= (buf_size - (size_t)offset)) {
                        ESP_LOGE(tag, "CONNECT buffer overflow");
                        NATSUtil::secure_free(buf);
                        return;
            ...
            case NATS_AUTH_CREDENTIALS:
                ...
                if (auth_config.jwt != NULL) {
                    int n = snprintf(buf + offset, buf_size - offset, ",\"jwt\":\"%s\"", auth_config.jwt);
                    if (n < 0 || (size_t)n >= (buf_size - (size_t)offset)) {
                        ESP_LOGE(tag, "CONNECT buffer overflow");
                        NATSUtil::secure_free(buf);
                        return;
                ...
                if (auth_config.nkey_seed != NULL && auth_config.sign_fn != NULL && server_nonce[0] != '\0') {
                    ...
                    size_t sig_len = 0;
                    if (auth_config.sign_fn((const uint8_t *)server_nonce, strlen(server_nonce), signature, &sig_len,
                        ...
                        if (sig_len != 64) {
                            ...
                            if (b64_ret < 0) {
                                ...
                                int n = snprintf(buf + offset, buf_size - offset, ",\"sig\":\"%s\"", sig_b64);
                                if (n < 0 || (size_t)n >= (buf_size - (size_t)offset)) {
                                    ESP_LOGE(tag, "CONNECT buffer overflow");
                                    NATSUtil::secure_free(buf);
                                    return;
                                ...
                                if (auth_config.nkey_public != NULL) {
                                    int n2 = snprintf(buf + offset, buf_size - offset, ",\"nkey\":\"%s\"",
                                                      auth_config.nkey_public);
    ...
    void recv()
    {
        ...
        if (strncmp(buf, "INFO ", 5) == 0) {
            ...
            while (*json_str == ' ')
                json_str++; // Skip extra whitespace
        ...
        if (strcmp(argv[0], NATS_CTRL_MSG) == 0) {
            ...
            if (*endptr != '\0' || sid_long < 0 || sid_long > INT_MAX) {
                free(buf);
                return;
            }
            ...
            if (*endptr != '\0' || payload_size_long < 0 || payload_size_long > INT_MAX) {
                free(buf);
                return;
            }
            int payload_size = (int)payload_size_long;
            // Check for integer overflow and enforce max payload size
            if (payload_size < 0 || payload_size > NATS_MAX_PAYLOAD_SIZE) {
                free(buf);
                return;
            }
            ...
            msg e = {argv[1], sid, (argc == 5) ? argv[3] : "", NULL, 0, payload_buf, payload_size};
            ...
            if (*endptr != '\0' || header_size_long < 0 || header_size_long > INT_MAX) {
                free(buf);
                return;
            }
            int header_size = (int)header_size_long;
            // Enforce max header size
            if (header_size > NATS_MAX_HEADER_LEN) {
                free(buf);
                return;
            }

            long total_size_long = strtol((argc == 6) ? argv[5] : argv[4], &endptr, 10);
            if (*endptr != '\0' || total_size_long < 0 || total_size_long > INT_MAX) {
                free(buf);
                return;
            }
            int total_size = (int)total_size_long;
            // Check for integer overflow and enforce max total size
            if (total_size < 0 || total_size > NATS_MAX_PAYLOAD_SIZE + NATS_MAX_HEADER_LEN) {
                free(buf);
                return;
            }
            ...
            msg e = {argv[1], sid, (argc == 6) ? argv[3] : "", header_buf, header_size, data_buf, data_size};
            metrics.msgs_received++;
    ...
    {
        if (headers == NULL)
            return true; // NULL headers are allowed
    ...
    void buffer_message(const char *subject, const char *msg, const char *replyto, const char *headers)
    {
        ...
        if (subject != NULL && pmsg.subject == NULL)
            return;
    ...
    void restore_subscriptions()
    {
        ...
        if (restored > NATS_MAX_SUBSCRIPTIONS) {
            ESP_LOGW(tag, "Restored %zu subscriptions (exceeds limit %d) - server may reject some", restored,
     ...
     */
    int sub_internal(const char *subject, sub_cb cb, const char *queue = NULL, int max_wanted = 0,
                     unsigned long timeout_ms = 0, timeout_cb on_timeout = NULL, bool send_sub = true)
    {
        ...
        Sub *sub = new Sub(cb, max_wanted, timeout_ms, on_timeout, send_sub ? subject : NULL, send_sub ? queue : NULL);
        if (sub == NULL) {
            xSemaphoreGiveRecursive(mutex);
            last_error_code = NATS_ERR_OUT_OF_MEMORY;
            return -1;
  ...
  private:
    ...
    bool connect_with_transport(unsigned long timeout_ms)
    {
        ...
        do {
            ...
            ESP_LOGI(tag, "Attempting transport connect to %s:%d", server.hostname, server.port);
            ...
            if (transport->connect(&config)) {
                ...
                ESP_LOGI(tag, "Transport connected to %s:%d", server.hostname, server.port);
                ...
                while (!connected && (NATSUtil::millis() - start_time) < timeout_ms) {
                    ...
                    if (transport->has_data_available(100)) { // 100ms timeout
                        if (recv_mutex != NULL)
                            xSemaphoreTakeRecursive(recv_mutex, portMAX_DELAY);
                        recv(); // Process INFO message, send CONNECT
                        if (recv_mutex != NULL)
                            xSemaphoreGiveRecursive(recv_mutex);
                    }
                    vTaskDelay(pdMS_TO_TICKS(10));
                }
  ...
  public:
    bool connect(unsigned long timeout_ms = 5000)
    {
        ...
        size_t start_idx = current_server_idx;
        do {
            ESP_LOGI(tag, "Attempting to connect to %s:%d", servers[current_server_idx].hostname,
            ...
            if (try_connect_to_server(servers[current_server_idx])) {
                ...
                connect_state = CONNECTING;
                metrics.last_connect_time = NATSUtil::millis();
                ESP_LOGI(tag, "Connected to %s:%d", servers[current_server_idx].hostname,
                         servers[current_server_idx].port);
    ...
    void disconnect()
    {
        if (!connected && (transport == NULL || !transport->is_connected()))
            return;
    ...
    void publish(const char *subject, const char *msg = NULL, const char *replyto = NULL)
    {
        ...
        if (replyto != NULL && replyto[0] != '\0') {
            send_fmt("PUB %s %s %lu", subject, replyto, (unsigned long)msg_len);
        } else {
            send_fmt("PUB %s %lu", subject, (unsigned long)msg_len);
        }
        send((msg == NULL) ? "" : msg);
        metrics.msgs_sent++;
    }
    ...
    void publish_with_headers(const char *subject, const char *headers, const char *msg = NULL,
    ...
    int request_with_timeout(const char *subject, const char *msg, sub_cb cb, unsigned long timeout_ms,
                             timeout_cb on_timeout = NULL, const int max_wanted = 1)
    {
        if (subject == NULL || subject[0] == 0)
            return -1;
        if (!connected)
    ...
    int jetstream_publish(const char *subject, const char *msg, sub_cb ack_cb, timeout_cb on_timeout = NULL,
                          unsigned long timeout_ms = 5000, const char *msg_id = NULL)
    {
        if (subject == NULL || subject[0] == 0)
            return -1;
        if (!connected)
                                ...
                                timeout_cb on_timeout = NULL, unsigned long timeout_ms = 5000)
    {
        ...
        if (config->subjects != NULL) {
            ...
            if (subject_count >= NATS_MAX_SUBJECTS) {
                ESP_LOGW(tag, "Stream subjects reached limit (%d) - array may not be NULL-terminated",
                         NATS_MAX_SUBJECTS);
            }
        }
        offset += snprintf(json + offset, json_size - offset, "]");
    ...
    int jetstream_stream_delete(const char *stream_name, sub_cb response_cb, timeout_cb on_timeout = NULL,
    ...
    int jetstream_stream_info(const char *stream_name, sub_cb response_cb, timeout_cb on_timeout = NULL,
                              unsigned long timeout_ms = 5000)
    {
        if (stream_name == NULL)
            return -1;
    ...
    int jetstream_consumer_create(const jetstream_consumer_config_t *config, sub_cb response_cb,
                                  timeout_cb on_timeout = NULL, unsigned long timeout_ms = 5000)
    {
        if (config == NULL || config->stream_name == NULL)
            return -1;
        if (!connected)
            return -1;
        ...
        char api_subject[1024];
        if (config->durable_name != NULL) {
            snprintf(api_subject, sizeof(api_subject), "$JS.API.CONSUMER.CREATE.%s.%s", config->stream_name,
                     config->durable_name);
        } else {
    ...
    int jetstream_consumer_delete(const char *stream_name, const char *consumer_name, sub_cb response_cb,
                                  timeout_cb on_timeout = NULL, unsigned long timeout_ms = 5000)
    {
        ...
        char api_subject[1024];
        snprintf(api_subject, sizeof(api_subject), "$JS.API.CONSUMER.DELETE.%s.%s", stream_name, consumer_name);

        return request_with_timeout(api_subject, "", response_cb, timeout_ms, on_timeout, 1);
    }

    // Pull messages from a JetStream consumer
    int jetstream_pull(const char *stream_name, const char *consumer_name, int batch_size, sub_cb message_cb,
                       timeout_cb on_timeout = NULL, unsigned long timeout_ms = 5000, bool no_wait = true)
    {
        if (stream_name == NULL || consumer_name == NULL || batch_size <= 0)
            return -1;
        if (!connected)
            return -1;
        ...
        char api_subject[1024];
        snprintf(api_subject, sizeof(api_subject), "$JS.API.CONSUMER.MSG.NEXT.%s.%s", stream_name, consumer_name);
    ...
    {
        if (reply_subject == NULL || reply_subject[0] == 0)
            return;
        if (!connected)
            return;
    ...
    int jetstream_fetch(const char *stream_name, const char *consumer_name,
                        const jetstream_fetch_request_t *fetch_config, sub_cb message_cb, timeout_cb on_timeout = NULL,
                        unsigned long timeout_ms = 5000)
    {
        if (stream_name == NULL || consumer_name == NULL || fetch_config == NULL)
            return -1;
        if (fetch_config->batch <= 0)
            return -1;
        if (!connected)
            return -1;
        ...
        if (fetch_config->heartbeat > 0) {
            offset +=
                snprintf(json + offset, sizeof(json) - offset, ",\"idle_heartbeat\":%lld", fetch_config->heartbeat);
        }
    ...
    int jetstream_account_info(sub_cb response_cb, timeout_cb on_timeout = NULL, unsigned long timeout_ms = 5000)
    {
        if (!connected)
            return -1;
    ...
    int jetstream_consumer_create_ordered(const char *stream_name, const char *filter_subject, sub_cb response_cb,
                                          timeout_cb on_timeout = NULL, unsigned long timeout_ms = 5000)
    {
        if (stream_name == NULL)
            return -1;
    ...
    int kv_create_bucket(const kv_config_t *config, sub_cb response_cb, timeout_cb on_timeout = NULL,
                         unsigned long timeout_ms = 5000)
    {
        if (config == NULL || config->bucket == NULL)
            return -1;
        if (!connected)
            return -1;
        ...
        int offset = snprintf(json, json_size, "{\"name\":\"KV_%s\",\"subjects\":[\"$KV.%s.>\"]", escaped_bucket,
                              escaped_bucket);
        if (offset >= (int)json_size) {
            free(json);
            return -1;
    ...
    int kv_get(const char *bucket, const char *key, sub_cb response_cb, timeout_cb on_timeout = NULL,
               unsigned long timeout_ms = 5000)
    {
    ...
    int kv_put(const char *bucket, const char *key, const char *value, sub_cb ack_cb, timeout_cb on_timeout = NULL,
               unsigned long timeout_ms = 5000)
    {
        if (bucket == NULL || key == NULL)
            return -1;
    ...
    int kv_delete(const char *bucket, const char *key, sub_cb ack_cb, timeout_cb on_timeout = NULL,
                  unsigned long timeout_ms = 5000)
    {
        if (bucket == NULL || key == NULL)
            return -1;
    ...
    int kv_purge(const char *bucket, const char *key, sub_cb ack_cb, timeout_cb on_timeout = NULL,
                 unsigned long timeout_ms = 5000)
    {
        if (bucket == NULL || key == NULL)
            return -1;
    ...
    int kv_keys(const char *bucket, sub_cb response_cb, timeout_cb on_timeout = NULL, unsigned long timeout_ms = 5000)
    {
        if (bucket == NULL)
            return -1;
        if (!connected)
    ...
    {
        if (bucket == NULL || key_pattern == NULL)
            return -1;
        if (!connected)
            return -1;
    ...
    int kv_history(const char *bucket, const char *key, sub_cb response_cb, timeout_cb on_timeout = NULL,
                   unsigned long timeout_ms = 5000)
    {
        if (bucket == NULL || key == NULL)
            return -1;
        ...
        jetstream_consumer_config_t consumer_config = {.stream_name = stream_name,
    ...
    int obj_create_bucket(const object_store_config_t *config, sub_cb response_cb, timeout_cb on_timeout = NULL,
                          unsigned long timeout_ms = 5000)
    {
        if (config == NULL || config->bucket == NULL)
            return -1;
        ...
        int len = snprintf(payload, sizeof(payload),
                           ...
                           "\"num_replicas\":%d,"
                           "\"discard\":\"new\"",
                           stream_name, escaped_bucket, escaped_bucket, escaped_storage,
        ...
        if (config->description != NULL) {
            ...
            len += snprintf(payload + len, sizeof(payload) - len, ",\"description\":\"%s\"", escaped_desc);
        }

        if (config->max_bytes > 0) {
            len += snprintf(payload + len, sizeof(payload) - len, ",\"max_bytes\":%zu", config->max_bytes);
        }

        if (config->ttl > 0) {
            len += snprintf(payload + len, sizeof(payload) - len, ",\"max_age\":%lld", (long long)config->ttl);
        }
...
#pragma GCC diagnostic pop
     ...
     */
    int obj_put(const char *bucket, const char *name, const uint8_t *data, size_t data_len, const char *description,
                sub_cb ack_cb, timeout_cb on_timeout = NULL, unsigned long timeout_ms = 5000)
    {
        if (bucket == NULL || name == NULL || data == NULL || data_len == 0)
            return -1;
        if (!connected)
            return -1;
        ...
        if (data_len > SIZE_MAX - CHUNK_SIZE + 1)
            return -1;
        ...
        for (uint64_t i = 0; i < num_chunks; i++) {
            ...
            size_t chunk_len = (chunk_offset + CHUNK_SIZE > data_len) ? (data_len - chunk_offset) : CHUNK_SIZE;

            char chunk_subject[1024];
            snprintf(chunk_subject, sizeof(chunk_subject), "$O.%s.C.%s", bucket, nuid);
        ...
        int len = snprintf(meta_payload, sizeof(meta_payload),
                           ...
                           escaped_name, data_len, (unsigned long long)num_chunks, escaped_nuid, CHUNK_SIZE);
        ...
        if (description != NULL) {
            ...
            len += snprintf(meta_payload + len, sizeof(meta_payload) - len, ",\"description\":\"%s\"", escaped_desc);
            if (len < 0 || (size_t)len >= sizeof(meta_payload)) {
        ...
        // Add timestamp
        len += snprintf(meta_payload + len, sizeof(meta_payload) - len, ",\"mtime\":%llu}",
                        (unsigned long long)(NATSUtil::millis() * 1000000ULL));
        if (len < 0 || (size_t)len >= sizeof(meta_payload)) {
    ...
    int obj_get_info(const char *bucket, const char *name, sub_cb response_cb, timeout_cb on_timeout = NULL,
                     unsigned long timeout_ms = 5000)
    {
        ...
        if (ret >= (int)sizeof(meta_subject))
            return -1;
        ...
        char payload[512];
        ret = snprintf(payload, sizeof(payload), "{\"last_by_subj\":\"%s\"}", escaped_meta_subject);
        if (ret >= (int)sizeof(payload))
            return -1;

        char api_subject[1024];
        ret = snprintf(api_subject, sizeof(api_subject), "$JS.API.STREAM.MSG.GET.%s", stream_name);
        if (ret >= (int)sizeof(api_subject))
            return -1;
     ...
     */
    int obj_get(const char *bucket, const char *name, sub_cb chunk_cb, timeout_cb on_timeout = NULL,
    ...
    int obj_list(const char *bucket, sub_cb response_cb, timeout_cb on_timeout = NULL, unsigned long timeout_ms = 5000)
    {
        if (bucket == NULL)
            return -1;
        if (!connected)
    ...
    int obj_delete(const char *bucket, const char *name, sub_cb ack_cb, timeout_cb on_timeout = NULL,
                   unsigned long timeout_ms = 5000)
    {
        if (bucket == NULL || name == NULL)
            return -1;
    ...
    int jetstream_direct_get(const char *stream_name, uint64_t seq, sub_cb response_cb, timeout_cb on_timeout = NULL,
                             unsigned long timeout_ms = 5000)
    {
        if (stream_name == NULL)
            return -1;
    ...
    int jetstream_direct_get_last(const char *stream_name, const char *subject, sub_cb response_cb,
                                  timeout_cb on_timeout = NULL, unsigned long timeout_ms = 5000)
    {
        if (stream_name == NULL || subject == NULL)
            return -1;
        if (!connected)
            return -1;
    ...
    int jetstream_consumer_pause(const char *stream_name, const char *consumer_name, uint64_t pause_until_ns,
                                 sub_cb response_cb, timeout_cb on_timeout = NULL, unsigned long timeout_ms = 5000)
    {
        if (stream_name == NULL || consumer_name == NULL)
            return -1;
        if (!connected)
        ...
        char subject[256];
        snprintf(subject, sizeof(subject), "$JS.API.CONSUMER.PAUSE.%s.%s", stream_name, consumer_name);

        char payload[128];
        snprintf(payload, sizeof(payload), "{\"pause_until\":%llu}", (unsigned long long)pause_until_ns);

        return request_with_timeout(subject, payload, response_cb, timeout_ms, on_timeout);
    }

    // Resume paused consumer (convenience method)
    int jetstream_consumer_resume(const char *stream_name, const char *consumer_name, sub_cb response_cb,
                                  timeout_cb on_timeout = NULL, unsigned long timeout_ms = 5000)
    {
        // Resume by setting pause_until to 0
        return jetstream_consumer_pause(stream_name, consumer_name, 0, response_cb, on_timeout, timeout_ms);
    }
    ...
    const char *error_string(nats_error_code_t err) const
    {
        switch (err) {
            ...
            case NATS_ERR_PROTOCOL_ERROR:
                return "Protocol error";
            case NATS_ERR_MAX_PINGS_EXCEEDED:
                return "Maximum outstanding pings exceeded";
            case NATS_ERR_DISCONNECTED:
                return "Disconnected";
            case NATS_ERR_INVALID_SUBJECT:
                return "Invalid subject";
            case NATS_ERR_NOT_CONNECTED:
                return "Not connected";
            case NATS_ERR_INVALID_CONFIG:
    ...
    void process()
    {
        if (log_tick_count++ % 1000 == 0) {
            // ESP_LOGI(tag, "(tick %d) Outstanding pings: %d, Reconnect attempts: %d", log_tick_count,
            // outstanding_pings, reconnect_attempts);
        }
        ...
        if (transport != NULL && transport->is_connected()) {
            ...
            for (size_t i = 0; i < timed_out_sids.size(); i++) {
                ...
                xSemaphoreTakeRecursive(mutex, portMAX_DELAY);
                Sub *sub = (timed_out_sids[i] >= 0 && (size_t)timed_out_sids[i] < subs.size()) ? subs[timed_out_sids[i]]
                                                                                               : NULL;
